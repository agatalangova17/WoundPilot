    ...
    var body: some View {
        VStack(spacing: 12) {
            ...
            if let region = selectedRegion {
                Text(LocalizedStrings.bodyRegionLabel(region))
                    .font(.headline)
                    .padding(.top, 4)
            }
            ...
            Circle()
                ...
                .overlay(
                    Circle()
                        ...
                        .background(Circle().fill(isSelected ? Color.blue.opacity(0.25) : .clear))
                )
        }
        .buttonStyle(.plain)
        .position(x: nx * size.width, y: ny * size.height)
        .accessibilityLabel(Text(LocalizedStrings.bodyRegionLabel(key)))
    }
}
